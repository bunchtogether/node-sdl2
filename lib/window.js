...
class window extends EventEmitter {
	constructor(options = {}) {
		...
		this.on('_event', (event) => {
			...
			else if(event.type == SDL_events.SDL_EventType.SDL_FINGERDOWN) {
				let ret = parseFinger(event.tfinger)
				this.emit('fingerdown', ret)
			}
			else if(event.type == SDL_events.SDL_EventType.SDL_FINGERUP) {
				let ret = parseFinger(event.tfinger)
				this.emit('fingerup', ret)
			}
			else if(event.type == SDL_events.SDL_EventType.SDL_FINGERMOTION) {
				let ret = parseFinger(event.tfinger)
				this.emit('fingermotion', ret)
			}
		})
		...
		function parseFinger(tfinger) {
			return ({
				x: tfinger.x,
				y: tfinger.y,
				pressure: tfinger.pressure,
				fingerId: tfinger.fingerId
			})
		}
	}
